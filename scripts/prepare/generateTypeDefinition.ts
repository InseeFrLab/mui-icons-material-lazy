...
export function generateTypeDefinition(params: { iconsDirPath: string; targetTsFilePath: string }) {
    ...
    const iconFileNames = fs.readdirSync(iconsDirPath).filter(name => name.endsWith(".svg"));
    ...
    muiComponentNames.forEach((muiComponentName, i) => {
        const reconstitutedFileName = muiComponentNameToFileName(muiComponentName);
    ...
    fs.writeFileSync(targetTsFilePath, Buffer.from(tsCode, "utf8"));
}
